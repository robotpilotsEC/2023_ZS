...
float Mouse_X_Speed(void)
{
  float res;
	...
	Mouse_X_Last = rc.info->Ch[2];
	
  if(abs(MOUSE_X_MOVE_SPEED > Xmax))res = 0;
  else 
		res = SF(MOUSE_X_MOVE_SPEED,Keyboard.MouseSF.SFX,0);
	
	rc.info->Ch[2] = (0.7f * MOUSE_X_RATE * res + 0.3f * Mouse_X_Last);
	
	if(rc.info->Ch[2]> 660)rc.info->Ch[2]= 660;
	if(rc.info->Ch[2]<-660)rc.info->Ch[2]=-660;
...
float Mouse_Y_Speed(void)
{
  float res;
	...
	Mouse_Y_Last = rc.info->Ch[3];
	
  if(abs(MOUSE_Y_MOVE_SPEED > Ymax))res = 0;
  else
		res = -SF(MOUSE_Y_MOVE_SPEED,Keyboard.MouseSF.SFY,0);

	rc.info->Ch[3] = (0.7f * MOUSE_Y_RATE * res + 0.3f * Mouse_Y_Last);
	
	if(rc.info->Ch[3]> 660)rc.info->Ch[3]= 660;
	if(rc.info->Ch[3]<-660)rc.info->Ch[3]=-660;
	
	if(abs(rc.info->Ch[3]) < 1)rc.info->Ch[3] = 0;
...
void Mouse_FS(void)
{
	...
	rc.info->Ch[2] = (0.7f * MOUSE_X_RATE * Mouse_X_UPDATA + 0.3f * Mouse_X_Last);
  rc.info->Ch[3] = (0.7f * MOUSE_Y_RATE * Mouse_Y_UPDATA + 0.3f * Mouse_Y_Last);
